...
public class WuliaobiaoqianDayingActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public EditText ed_jiuwuliaobianma;
    ...
    public void onViewClicked(View view){
        switch (view.getId()) {
            case R.id.btn_connect:{
                startActivityForResult(new Intent(WuliaobiaoqianDayingActivity.this, BluetoothListActivity.class), BLUETOOTH_REQUEST_CODE);
                break;
            }
            case R.id.btn_daying:{
                if(!isConnected){
                    XToastUtil.showToast(WuliaobiaoqianDayingActivity.this,"请先连接打印机!");
                    break;
                }
                printLabel();
                break;
            ...
            case R.id.btn_duocidaying:{
                ...
                startActivityForResult(new Intent(WuliaobiaoqianDayingActivity.this, InputDayingNumberActivity.class), 777);
                break;
            }
            case R.id.btn_search: {
                if(!ButtonUtils.isFastDoubleClick(R.id.btn_search)){
                    ...
                    jiuwuliaobianma = ed_jiuwuliaobianma.getText().toString();
                    if((xinwuliaobianma == null || xinwuliaobianma != null && "".equals(xinwuliaobianma))&&
                            (jiuwuliaobianma == null || jiuwuliaobianma != null && "".equals(jiuwuliaobianma))){
                        XToastUtil.showToast(this,"新物料编码,旧物料编码都为空！无法查询！");
                        return;
                    }
                    LoadingStaticDialog.showLoadingDialog(WuliaobiaoqianDayingActivity.this, "操作中，请稍等！");
                    startTimer();
                    missDialog = true;
...
//                            "&token="+PreferenceHelper.getToken()+"&partNumber="+xinwuliaobianma;
                    String url = "http://172.16.1.131:9090/api/app/dataWms?userName="+PreferenceHelper.getUserId()+
                            "&token="+PreferenceHelper.getToken();
                    if(!(xinwuliaobianma == null || xinwuliaobianma != null && "".equals(xinwuliaobianma))){
                        url = url +"&code="+xinwuliaobianma;
                    }
                    if(!(jiuwuliaobianma == null || jiuwuliaobianma != null && "".equals(jiuwuliaobianma))){
                        url = url +"&bismt="+jiuwuliaobianma;
                    }
                    ...
                    call.enqueue(new Callback() {
                        ...
                        public void onFailure(Call call, IOException e) {
                            Looper.prepare();
                            XToastUtil.showToast(WuliaobiaoqianDayingActivity.this,"失败！"+e.getLocalizedMessage());
                            missDialog = false;
                        ...
                        public void onResponse(Call call, Response response) throws IOException {
                            ...
                            try {
                                ...
                                if(codeResult != 200){
                                    ...
                                    LoadingStaticDialog.loadDialogDismiss();
                                    stopTimer();
                                    XToastUtil.showToast(WuliaobiaoqianDayingActivity.this,msg);
                                    Looper.loop();
                                    return;
                                }
                                String data = json.optString("data");
                                JSONObject jsonData = new JSONObject(data);
                                xinwuliaobianma = jsonData.optString("code");//新物料号
                                jiuwuliaobianma = jsonData.optString("bismt");//旧物料号
                                String wuliaomingchengandguigexinghao = jsonData.optString("name");//物料名称
                                ...
                                tuhaoguige = arr[1];
                                danwei = jsonData.optString("meins");//单位
                                Looper.prepare();
                                missDialog = false;
                                LoadingStaticDialog.loadDialogDismiss();
                                stopTimer();
                                //设置值
                                ed_wuliaobianma.setText(xinwuliaobianma);
                                tv_danwei.setText(danwei);
                                ed_jiuwuliaobianma.setText(jiuwuliaobianma);
            ...
            if (DeviceConnFactoryManager.ACTION_CONN_STATE.equals(action)) {
                ...
                switch (state) {
                    ...
                    case DeviceConnFactoryManager.CONN_STATE_CONNECTED:
                        ...
                        isConnected = true;
                        Toast.makeText(WuliaobiaoqianDayingActivity.this, "已连接", Toast.LENGTH_SHORT).show();
                        break;
                    case CONN_STATE_FAILED:
                        tv_state.setText("未连接");
                        Toast.makeText(WuliaobiaoqianDayingActivity.this, "连接失败！重试或重启打印机试试", Toast.LENGTH_SHORT).show();
                        break;
                }
                /* Usb连接断开、蓝牙连接断开广播 */
            ...
            switch (msg.what) {
                case CONN_STATE_DISCONN:
                    if (DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id] != null || !DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getConnState()) {
                        ...
                        Toast.makeText(WuliaobiaoqianDayingActivity.this, "成功断开连接", Toast.LENGTH_SHORT).show();
                    }
                    break;
                case PRINTER_COMMAND_ERROR:
                    Toast.makeText(WuliaobiaoqianDayingActivity.this, "请选择正确的打印机指令", Toast.LENGTH_SHORT).show();
                    break;
                case CONN_PRINTER:
                    Toast.makeText(WuliaobiaoqianDayingActivity.this, "请先连接打印机", Toast.LENGTH_SHORT).show();
                    break;
            }
        }
    };
    ...
    public  void startTimer() {
        ...
        countDownTimer = new CountDownTimer(30000L, 1000L) {
            ...
            public void onFinish() {
                if(missDialog == true){
                    LoadingStaticDialog.loadDialogDismiss();
                    XToastUtil.showToast(WuliaobiaoqianDayingActivity.this, "请求超时！");
                    stopTimer();
                }
            }
        }.start();
